...
function TreatmentsList(props) {
  ...
  const applyFilters = () => {
    ...
    if (tempPersonFilter) addClick(`users/${user.userId}`, "filter-person", new Date());
    if (tempRemoteFilter) addClick(`users/${user.userId}`, "filter-remote", new Date());
    if (tempAccessFilter) addClick(`users/${user.userId}`, "filter-access", new Date());
    if (tempCostFilter) addClick(`users/${user.userId}`, "filter-cost", new Date());
